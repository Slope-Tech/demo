...
const Checkout: React.FC<{
  ...
  setCustomerForm: any}> = ({
    ...
    const onPay = async () => {
      ...
      console.log('customerId & secret', customerJson.customer.id, customerJson.secret);
      ...
      window.initializeSlope({
        ...
        allowedTerms: customerForm.payNow ? null : process.env.NEXT_PUBLIC_PAY_LATER_TERMS?.split(','),
    ...
    const slopeButton = (
      <Button leftIcon={<img alt="Slope Logo" src="/images/slope_logo_white.png" height={18} />} fullWidth color="orange" loading={loading} onClick={onPay}>
        {customerForm.payNow ? 'Pay with Slope' : 'Pay later with Slope' }
      </Button>
    );
    ...
    return (
      <>
        ...
        <Grid gutter="xl">
          ...
          <Grid.Col md={12} lg={8}>
            <Title order={3} mb="xs">Customer</Title>
            <ErrorAlert error={error} setError={setError} />
            <CustomerForm customerForm={customerForm} setCustomerForm={setCustomerForm} />
            <Title mt="lg" mb="sm" order={3}>Payment</Title>
            {customerForm.payNow ? (
              slopeButton
            ) : (
              <Grid align="center" gutter="xs" columns={11}>
                <Grid.Col md={11} lg={5}>
                  <Button leftIcon={<IconCreditCard />} fullWidth onClick={() => alert('Try paying with Slope!')}>
                    Pay now
                  </Button>
                </Grid.Col>
                <Grid.Col md={11} lg={1}>
                  <Text fw={700} sx={{ textAlign: 'center' }}>OR</Text>
                </Grid.Col>
                <Grid.Col md={11} lg={5}>
                  {slopeButton}
                </Grid.Col>
